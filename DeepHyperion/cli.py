...
from mapelites_uav import MapElitesUAV
...
if __name__ == "__main__":
    ...
    try:
        print("start")
        args = arg_parse()
        map_E = MapElitesUAV(case_study_file=args.test, budget=args.budget, logs=False)
        test_cases = map_E.run()

        ### copying the test cases to the output folder
        tests_fld = f'{TESTS_FOLDER}{time_stamp}/'
        Path(tests_fld).mkdir(parents=True, exist_ok=True)
